...
export abstract class Texture extends GlClosure<TexturePatch, [GlVec3]> {
  constructor() {
    super("getTexturePatch", TexturePatch.default(), [GlVec3.default()]);
  }
...
export class Constant extends glClosure.Constant<TexturePatch, [GlVec3]> {
  constructor(albedo: Vec3, roughness: number, specular: number) {
    super("getTexturePatch", new TexturePatch(albedo, roughness, specular), [GlVec3.default()]);
  }
...
export class Add extends glClosure.Reduce<TexturePatch, [GlVec3]> {
  constructor(lhs: GlClosure<TexturePatch, [GlVec3]>, rhs: GlClosure<TexturePatch, [GlVec3]>[]){
    super("getTexturePatch", new glClosure.Add("add", TexturePatch.default()), lhs, rhs);
  }
}
export class MulScalar extends glClosure.MulScalar<TexturePatch, [GlVec3]> {
  constructor(scale: number, v: GlClosure<TexturePatch, [GlVec3]>){
    super("getTexturePatch", scale, v);
  }
}

export class MulScalarField extends Texture {
  original: GlClosure<TexturePatch, [GlVec3]>;
  scale: GlClosure<GlFloat, [GlVec3]>;
  constructor(original: GlClosure<TexturePatch, [GlVec3]>, scale: GlClosure<GlFloat, [GlVec3]>) {
    super();
  ...
  override GlFunc_getTexturePatch(): string {return `
    TexturePatch ${this.glFuncName}(vec3 point) {
      TexturePatch res = ${this.original.glFuncName}(point);
